...
for day in [day]:
    for hour in np.arange(14,20,1):
        ...
        for minute in minutes:
            ...
            fName = os.path.join(outPath,"WRF_{0}_SPATIAL_AVERAGED_PROFILES_{1:%Y-%m-%d_%H:%M:%S}.csv".format(prefix,datetime))
            print(fName)
            df.to_csv(fName)
            ...
            fName = os.path.join(outPath,"WRF_{0}_SPATIAL_AVERAGED_2D_{1:%Y-%m-%d_%H:%M:%S}.csv".format(prefix,datetime))   
            a.to_csv(fName)
